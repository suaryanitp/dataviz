...
``` r
dim(iris) # jumlah baris dan kolom
```
...
``` r
head(iris) # melihat 6 baris teratas
```
...
``` r
tail(iris) # melihat 6 baris terbawah
```
...
``` r
summary(iris) # melihat ringkasan data
```
...
``` r
class(iris$Species) # melihat class species
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # jumlah kolom spesies
```

    ## [1] 150

``` r
levels(iris$Species) # melihat level pada spesies
```

    ## [1] "setosa"     "versicolor" "virginica"

``` r
length(levels(iris$Species)) # jumah level pada spesies
```

    ## [1] 3

``` r
unique(iris$Species) # menunjukan spesies yang unik
```
...
``` r
length(unique(iris$Species)) # jumlah data spesies yang unik
```

    ## [1] 3

``` r
mean(iris$Sepal.Length) # menunjukan mean 
```

    ## [1] 5.843333

``` r
sd(iris$Sepal.Length) # menunjukan sd
```

    ## [1] 0.8280661

``` r
median(iris$Sepal.Width) # menunjukan median
```

    ## [1] 3

``` r
sum(iris$Petal.Length) # menunjukan jumlah
```

    ## [1] 563.7

``` r
cor(iris[, -5]) # menghitung relasi antara variable numerik dan iris 
```
